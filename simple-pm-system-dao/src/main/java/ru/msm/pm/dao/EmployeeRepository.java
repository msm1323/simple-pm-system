...
public interface EmployeeRepository extends JpaRepository<Employee, Long>, JpaSpecificationExecutor<Employee> {
    ...
    @Query(value = "UPDATE employee set name=:#{#e.name}, surname=:#{#e.surname}, status=:#{#e.status.name()}," +
            " patronymic=:#{#e.patronymic}, account=:#{#e.account}, position=:#{#e.position}, email=:#{#e.email}" +
